#!/usr/bin/env python3
"""W5 results aggregator.

The SGLang driver writes results/seed_<S>/cell_<X>/rate_<R>/<policy>.json.
The vLLM driver writes results_vllm/seed_<S>/cell_<X>/<policy>.json (no
rate sub-directory; W5 vLLM uses a single calibrated load per cell).

Produces:
  - summary.csv     : one row per completed run
  - aggregated.csv  : median / mean / stdev across seeds per (cell x rate x policy)
  - delta.csv       : relative improvement (%) of each non-baseline policy vs the baseline
                     per (cell x rate), using median across seeds

For the vLLM tree pass:
  --results-dir benchmarks/w5/results_vllm  --baseline fcfs_apc_lru
"""
...
def parse_path(path: str) -> tuple:
    # SGLang : .../seed_42/cell_C_long/rate_heavy/clpm_gm_dl_pe.json
    # vLLM   : .../seed_42/cell_C_long/clpm_gm_dl_pe.json   (no rate level)
    parts = path.split(os.sep)
    seed = int(next(x for x in parts if x.startswith("seed_")).split("_", 1)[1])
    cell_token = next(x for x in parts if x.startswith("cell_"))
    cell = cell_token.split("_", 1)[1]
    rate_match = next((x for x in parts if x.startswith("rate_")), None)
    rate = rate_match.split("_", 1)[1] if rate_match else "single"
...
def main() -> None:
    ...
    ap.add_argument("--results-dir", default=str(W5 / "results"))
    ap.add_argument("--out-dir", default=str(W5))
    ap.add_argument("--baseline", default="lpm_lru",
                    help="policy name used as reference in delta.csv "
                         "(default lpm_lru; pass fcfs_apc_lru for the vLLM tree)")
    args = ap.parse_args()

    files = sorted(
        glob.glob(os.path.join(args.results_dir, "seed_*/cell_*/rate_*/*.json"))
        + glob.glob(os.path.join(args.results_dir, "seed_*/cell_*/*.json"))
    )
    # Filter out duplicates (rate-tree files would otherwise show twice if both
    # globs match) and any internal "_*" files.
    files = sorted({p for p in files if not os.path.basename(p).startswith("_")})
    if not files:
