...
- `stress_long_paths_shared_prefixes` — workload shaped like Scenario B: 10 agents × 50 sessions × 5 turns with a 50-token SP, then random-delete half. Invariants hold throughout.
...
- `pending_demand` and `terminators_at` at the Python boundary match naive
  ...
  against an agent-sessions-shaped tree (50 agents × 3 sessions × 5 turns).
...
Workload: 30 sessions × 3 turns × 20 agent types, SP=1500 tokens, λ=0.5/s,
...
1. **Peek's pending tree stays in exact sync with sglang's waiting queue.**
   ...
   with SP=1500 + 30 sessions × 3 turns) leave zero residue in either
   direction.
