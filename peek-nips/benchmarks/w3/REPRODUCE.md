...
(SGLang 0.5.9, vLLM 0.19.1) at both topologies (DP=1 on 2×H100, DP=2 on
4×H100) for both cells (B=long-doc RAG, C=shared-prompt chat).
...
- **DP=1 only**: 2 × H100 80GB
- **DP=1 + DP=2**: 4 × H100 80GB
...
### 4a. SGLang DP=1 (2×H100, single TP=2 replica)
...
### 4b. SGLang DP=2 (4×H100, two TP=2 replicas behind sglang-router)
...
### 4d. vLLM DP=2 (4×H100, two TP=2 vLLM workers behind sglang-router)
...
Output is one row per (engine × dp × cell × policy × metric) with the
...
Per (engine × topology) combination, single seed:

| Stage | Approx. time |
|---|---|
| Model load (2×H100 first time, slow FS) | 15–25 min × 2 servers |
| Model load (subsequent, page cache warm) | 5–10 min × 2 servers |
| Cell C bench (1000 reqs) | 3–10 min |
| Cell B bench (500 reqs) | 7–20 min |
| **Total per (engine × dp), 1 seed** | **45–90 min** |
| **Total all 4 × 1 seed** | **3–6 hr** |
| **Total all 4 × 3 seeds (paper config)** | **9–18 hr** |
