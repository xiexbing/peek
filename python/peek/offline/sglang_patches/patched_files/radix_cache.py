...
class TreeNode:
    ...
    def __init__(self, id: Optional[int] = None, priority: int = 0):
        ...
        # queue-aware eviction: number of waiting-queue requests referencing
        # this node (or an ancestor/descendant sharing its prefix blocks)
        self.queue_ref_count = 0
        # cached depth: total prefix length from root to this node
        # maintained during insert/split to avoid O(depth) walk in eviction
        self.depth = 0
...
class RadixCache(BasePrefixCache):
    def __init__(self, params: CacheInitParams):
        ...
        elif self.eviction_policy == "queue-aware":
            self.eviction_strategy: EvictionStrategy = QueueAwareStrategy()
        elif self.eviction_policy == "lrfu":
            self.eviction_strategy: EvictionStrategy = LRFUStrategy()
        else:
            raise ValueError(
                f"Unknown eviction policy: {self.eviction_policy}. Supported policies: 'lru', 'lfu', 'fifo', 'mru', 'filo', 'priority', 'queue-aware', 'lrfu'."
            )
    ...
    def _match_prefix_helper(self, node: TreeNode, key: RadixKey, update_lru: bool = True):
        if update_lru:
    ...
    def _insert_helper(self, node: TreeNode, key: RadixKey, value, priority: int = 0):
        ...
        if len(key):
            ...
            new_node.depth = node.depth + len(key)
